...
export class EnemyDash extends Enemy {
    ...
    render(ctx) {
        ...
        if (this.#dashTask._animationTimer + this.#dashTask._dashTimer > 0) {
            const animTimer = this.#dashTask._animationTimer;
            const animDuration = Math.max(0.0001, this.#dashTask.animation);
            const progress = 1 - Math.min(1, animTimer / animDuration);
            if (progress > 0) {
                const r = this.radius * progress;
                fillCircle(ctx, this.pos.x, this.pos.y, r, "rgba(175, 38, 38, 1)", false);
            }
        }
    }
}
